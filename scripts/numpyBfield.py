...
print('Converting CST E-field ASCII to 3D matrices and save  in Numpy file')
...
Exyz=Efield[:,:,:,:,0]
#np.sum(Efield,4)  # combined fields Ex, Ey, Ez 
Eabs=np.abs(np.sum(Exyz*Exyz,3))  # Ex^2+Ey^2+Ez^2  

plt.hist(Eabs.flatten(), bins=200)
plt.show()
print('Writing to file : ',npfile)
nrrd.write("efield.nrrd", Eabs)
np.save(npfile, Efield)