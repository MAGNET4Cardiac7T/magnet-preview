...
class UNETR(nn.Module):
    def __init__(self, input_dim=6, output_dim=6, img_shape=(128, 80, 96), embed_dim=768, patch_size=16, num_heads=12, dropout=0.1):
        super().__init__()
        ...
        self.patch_dim = [int(x / p) for x, p in zip(img_shape, patch_size)]
    ...
    def forward(self, x):
        # TODo - find a better way to deal with shapes, this is very inefficient
        diffZ = self.img_shape[0] - x.size()[2]
        diffY = self.img_shape[1] - x.size()[3]
        diffX = self.img_shape[2] - x.size()[4]
        ...
        output = output[:,:, (diffZ // 2):self.img_shape[0]-(diffZ - diffZ // 2), (diffY // 2):self.img_shape[1]-(diffY - diffY // 2),(diffX // 2):self.img_shape[2]-(diffX - diffX // 2)]
        return output
1   